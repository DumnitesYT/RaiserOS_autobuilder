...
fetch_rom_info() {
  local sysdir="$1"
  local prop="$sysdir/system/build.prop"
  [[ -f "$prop" ]] || prop="$sysdir/build.prop"
  [[ -f "$prop" ]] || { warn "build.prop не найден"; return; }
  ROM_DEVICE=$(grep -m1 "ro.product.device=" "$prop" | cut -d= -f2)
  ROM_VERSION=$(grep -m1 "ro.build.version.ota=" "$prop" | cut -d= -f2)
  [[ -z "$ROM_VERSION" ]] && \
    ROM_VERSION=$(grep -m1 "ro.build.display.id=" "$prop" | cut -d= -f2)
  ROM_BUILD_DATE=$(grep -m1 "ro.build.date.utc=" "$prop" | cut -d= -f2 | \
    xargs -I{} date -d @{} +%Y%m%d 2>/dev/null || date +%Y%m%d)
